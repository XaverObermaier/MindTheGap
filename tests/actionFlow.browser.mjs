...
assert.equal(await evaluate(`document.querySelectorAll('.attention-gap').length`), 1);
assert.match(await evaluate(`document.querySelector('.attention-gap').textContent`), /ranked #1/);
assert.match(await evaluate(`document.querySelector('.attention-gap').textContent`), /Norwegian Refugee Council/);
assert.equal(await evaluate(`(() => { const link = document.querySelector('.attention-gap a'); return [link?.tagName, link?.target, link?.rel, link?.href].join('|'); })()`), 'A|_blank|noopener|https://www.nrc.no/feature/2026/the-worlds-most-neglected-displacement-crises-2025');
assert.equal(await evaluate(`(() => { const link = document.querySelector('.attention-gap a'); link.focus(); return document.activeElement === link; })()`), true);
assert.doesNotMatch(await evaluate(`document.querySelector('.attention-gap').textContent`), /combined score|needIndex/);
...
assert.equal(await evaluate(`document.querySelector('#action-context').hidden`),true);
await visit('/html/country.html?code=KEN', `document.querySelector('#country-detail .btn')`);
assert.equal(await evaluate(`document.querySelectorAll('.attention-gap').length`), 1);
assert.match(await evaluate(`document.querySelector('.attention-gap').textContent`), /not ranked in this edition/);
assert.match(await evaluate(`document.querySelector('.attention-gap').textContent`), /does not mean the crisis is not neglected/);
assert.equal(await evaluate(`document.querySelector('.attention-gap a')`), null);
assert.doesNotMatch(await evaluate(`document.querySelector('.attention-gap').textContent`), /ranked #/);
